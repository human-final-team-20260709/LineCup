...
export const Pagination = styled.div`display: flex; align-items: center; justify-content: center; gap: 16px; padding: 14px 16px; color: ${c.muted}; ${mono} font-size: 11px; > div { display: flex; gap: 5px; }`;
export const PageButton = styled.button`${focus} display: inline-grid; place-items: center; min-width: 32px; height: 32px; padding: 0 8px; border: 1px solid ${({ $active }) => ($active ? c.green : c.border)}; border-radius: 4px; background: ${({ $active }) => ($active ? 'rgba(75, 226, 119, .14)' : c.low)}; color: ${({ $active }) => ($active ? c.green : c.text)}; cursor: pointer; &:disabled { opacity: .35; cursor: not-allowed; }`;
