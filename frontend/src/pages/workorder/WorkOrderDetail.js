...
export default function WorkOrderDetail() {
  ...
  return (
    <Page>
      <Header>
        <div>
          <h1>{order.workOrderNo}</h1>
          <p>{order.productName} · 5초 주기 갱신</p>
        </div>
        <Button $secondary onClick={() => navigate("/work-orders/list")}>
          목록
        </Button>
      </Header>
      <Toolbar>
        ...
        {(actionForStatus[order.status] || []).map((action) => (
          <Button
            ...
            onClick={() => run(() => statusMutation.mutateAsync({ action }))}
          >
            {actionLabel[action]}
          </Button>
        ))}
      ...
      <Grid>
        <Card>
          <span>목표 수량</span>
          <h2>
            {formatNumber(order.targetQty)} {detail.productUnit}
          </h2>
        </Card>
        <Card>
          <span>현재 생산</span>
          <h2>{formatNumber(order.currentQty)}</h2>
          <progress max={order.targetQty} value={order.currentQty} />
        </Card>
        <Card>
          <span>정상 / 불량</span>
          <h2>
            {formatNumber(order.goodQty)} / {formatNumber(order.defectQty)}
          </h2>
        </Card>
        <Card>
          <span>작업 시각</span>
          <p>시작 {toKst(order.startedAt)}</p>
          <p>완료 {toKst(order.completedAt)}</p>
        </Card>
      </Grid>
          ...
          >
            <Select name="supervisorUserId" defaultValue={order.supervisorId}>
              {pageContent(supervisorsQuery.data)
                .filter(
                  (entry) =>
                    entry.active && entry.approvalStatus === "approved",
                )
                .map((entry) => (
      ...
      <Card>
        ...
        >
          <div>
            {pageContent(operatorsQuery.data)
              .filter(
              ...
              .map((entry) => (
                <label
                  key={entry.userId}
                  style={{ display: "inline-flex", marginRight: 18 }}
                >
                  <input
                    type="checkbox"
                    name="worker"
                    value={entry.userId}
                    defaultChecked={detail.workers.some(
                      (worker) => worker.userId === entry.userId,
                    )}
                  />
                  {entry.name}
                </label>
      ...
      <Grid>
        {detail.processes.map((process) => (
          <Card key={process.processProgressId}>
            ...
            <Badge
              $tone={toneForStatus(
                detail.equipments.find(
                  (equipment) => equipment.equipmentId === process.equipmentId,
                )?.status,
              )}
            >
              {detail.equipments.find(
                (equipment) => equipment.equipmentId === process.equipmentId,
              )?.statusLabel || "설비 상태 미확인"}
            </Badge>
          </Card>
        ))}
      </Grid>
