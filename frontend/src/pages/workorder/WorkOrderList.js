  ...
  return (
    <Page>
      <Header>
        <div>
          <h1>작업지시</h1>
          <p>서버 검색 결과를 5초마다 갱신합니다.</p>
        </div>
        <Button onClick={() => setShowForm((value) => !value)}>
          {showForm ? "등록 닫기" : "작업지시 등록"}
        </Button>
      </Header>
      <Toolbar>
        <Select
            ...
            <label>
              ...
              <Select name="supervisorUserId" required>
                ...
                {pageContent(supervisorsQuery.data)
                  .filter(
                    (user) => user.active && user.approvalStatus === "approved",
                  )
                  .map((user) => (
        ...
        <Grid>
          {rows.map((order) => (
            ...
            >
              ...
              <p>{order.workOrderNo}</p>
              <progress
                max={order.targetQty}
                value={order.currentQty}
                style={{ width: "100%" }}
              />
              <p>
                {formatNumber(order.currentQty)} /{" "}
                {formatNumber(order.targetQty)} EA ({order.progressRate}%)
              </p>
            ...
            <tbody>
              {rows.map((order) => (
                ...
                >
                  ...
                  <td>{formatNumber(order.hourlyTargetQty)}</td>
      ...
      <Toolbar>
        ...
        <span>
          {page + 1} / {workOrdersQuery.data?.totalPages || 1}
        </span>
        <Button
          $secondary
          disabled={page + 1 >= (workOrdersQuery.data?.totalPages || 1)}
          onClick={() => setPage((value) => value + 1)}
        >
          다음
        </Button>
